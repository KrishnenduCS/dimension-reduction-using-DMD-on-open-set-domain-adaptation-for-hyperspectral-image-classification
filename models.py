...
class Dense_Block(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.fc = nn.Linear(in_features, out_features)#Applies a linear transformation to the incoming data args are size of input and size of output
        self.relu = torch.nn.LeakyReLU()
...
class GradReverse(torch.autograd.Function):#We can implement our own custom autograd Functions by subclassing
...
class Generator_s2sa(nn.Module):
    def __init__(self):
        ...
        self.fc1 = Dense_Block(2048, 1000)
...
class Net(nn.Module):
    def __init__(self, task='s2sa'):
        ...
        if task == 's2sa':
            ...
            self.classifier = Classifier_s2sa(7)
        else:
